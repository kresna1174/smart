...
class Kampung extends CI_Controller{
	...
	function detail($slugs){
		...
		if($query->num_rows() > 0){
			...
			$data=$this->m_kampung->get_berita_by_kode($kode);
			$row=$data->row_array();
			$x['id']=$row['tulisan_id'];
			...
			$x['show_komentar']=$this->m_kampung->show_komentar_by_tulisan_id($kode);
			$x['category']=$this->db->get('tbl_kategori');
			$x['populer']=$this->db->query("SELECT * FROM tbl_tulisan ORDER BY tulisan_views DESC LIMIT 5");
			$this->load->view('bagian/header');
			$this->load->view('depan/v_kampung_detail',$x);
			$this->load->view('bagian/footer');
		}else{
			redirect('artikel');
		}
    ...
    function search(){
        ...
        $query=$this->m_kampung->cari_berita($keyword);
				if($query->num_rows() > 0){
					$x['data']=$query;
					$x['category']=$this->db->get('tbl_kategori');
  				$x['populer']=$this->db->query("SELECT * FROM tbl_tulisan ORDER BY tulisan_views DESC LIMIT 5");
